...
namespace PlakaKayitUygulamasi
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            LoadDataTable();
            // Grid satýrýna týklanýnca veri aktarma
            messageGridView.CellClick += (s, e) =>
            {
                if (e.RowIndex >= 0)
                {
                    GridRowClicked(messageGridView, this);
                }
            };

            // Ara butonuna týklandýðýnda arama fonksiyonunu çalýþtýr
            toolbar.SearchClicked += (s, e) =>
            {
                var (pSearchText, pSearchContains) = toolbar.GetSearchParameters();
                PerformSearch(pSearchText, pSearchContains);
            };
        }
        // Arama fonksiyonu
        private void PerformSearch(string searchText, bool xSearchContains)
        {
            GlbPlakaAra(searchText, xSearchContains);
            /*
            string filter = detailedSearch
                ? $"Plaka LIKE '%{searchText}%' OR AdSoyad LIKE '%{searchText}%' OR TelefonNo LIKE '%{searchText}%"
                : $"Plaka LIKE '%{searchText}%";

            (messageGridView.DataSource as BindingSource).Filter = filter;

            MessageBox.Show($"Arama tamamlandý: '{searchText}', Detaylý: {detailedSearch}", "Bilgi", MessageBoxButtons.OK, MessageBoxIcon.Information);
            */
        }
        // Grid üzerinde satýra týklandýðýnda Form1'deki kontrolleri dolduran metot
        ...
        {
            string plakaArama;
            GlbPlakaAra(textBoxPlakaArama?.Text,true);
        }

        private void GlbPlakaAra(string xPlaka,bool xPlakaAraContains = false)
        {
            
            if (string.IsNullOrEmpty(xPlaka))
            {
            ...
            try
            {
                using (var connection = new SQLiteConnection(connectionString))
                {
                    ...
                    {
                        command.Parameters.AddWithValue("@Plaka", xPlaka);
                ...
                if (!found)
                {
                    richTextBoxAramaSonucu.AppendText($"Plaka [Birebir] bulunamadý [{xPlaka}]");
                }
            }
            catch (Exception ex)
            {
            ...
            AddRecordToList(xPlaka, xPlakaAraContains);
        }

        private void AddRecordToList(string xPlaka,bool xPlakaAraContains)
        {
            LoadDataTable();
            bool found = false;
            try
            {
                using (var connection = new SQLiteConnection(connectionString))
                {
                    ...
                    string query = "";
                    if (xPlakaAraContains)
                        query = "SELECT * FROM PlakaKayitlari WHERE Plaka LIKE @Plaka";
                    else
                        query = "SELECT * FROM PlakaKayitlari WHERE Plaka = @Plaka";
                ...
                if (!found)
                {
                    ...
                    MessageBox.Show($"Plaka bulunamadý.{xPlaka}");
                }
            }
            catch (Exception ex)
            {
